...
```bash
python -m benchmark --systems bespoke --scale_factors 1 --query_ids 1,2 --benchmark tpch
```
...
```bash
python -m benchmark --systems bespoke,duckdb --scale_factors 1,5,20 --csv tpch.csv --benchmark tpch
```

## Notes

- The Bespoke system benchmarks the current `./output` workspace.
- Query IDs are resolved from benchmark definitions (`tpch`, `ceb`, or `spatial`), not from generated workspace files.
